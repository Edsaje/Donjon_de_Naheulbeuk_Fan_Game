...
/**
 * Composant de rendu 3D sp̩cialis̩ pour la sc̬ne d'exploration du Donjon (SRP).
 * G̬re la construction 3D des dalles, des murs massifs, des coffres et des escaliers.
 *
 * @author Hibouxe
 * @version 1.0
 */
public class DungeonSceneRenderer implements Disposable {
    ...
    public DungeonSceneRenderer(AssetProvider assetProvider, String themePath) {
        ...
        // D̩coupage automatique si l'image fait 256x256 (64x64 par frame)
    ...
    public void setLeaderClass(String className) {
        if (heroTexture != null) {
            heroTexture = null; // L'AssetProvider s'occupe de la gestion m̩moire
        }
    ...
    public void buildScene(Dungeon dungeon, fr.hibouxe.donjon_de_naheulbeuk_fan_game.model.entity.Team team, float playerX, float playerZ, int currentFloor) {
        ...
        // (Ligne de t̩l̩portation supprim̩e ici pour permettre la fluidit̩)
                ...
                if (cell.isWalkable()) {
                    ...
                    if (cell.hasChest()) {
                        if (chestBaseModel != null && chestLidModel != null) {
                            ...
                            // Param̬tres de la charni̬re du coffre (coordonn̩es depuis Blender)
                            ...
                            float hingeY = 0.5f;     // A MODIFIER: La hauteur (Z dans Blender) de ta charni̬re!
                            
                            // 1. Placement au centre
                            lid.transform.setToTranslation(posX + half, 0f, posZ + half);
                            // 2. D̩placement sur la charni̬re
                        ...
                        itemSprite.setTextureRegion(chestRegion); // TODO: Utiliser un sprite sp̩cifique pour l'item
                    ...
                    if (cell.hasDoor() && doorFrameModel != null && doorModel != null) {
                        ...
                        float hingeX = 0.493241f; // Coordonn̩e X de la charni̬re dans Blender
                        ...
                        // 3. D̩placement sur la charni̬re locale
                        ...
                        // 5. D̩placement inverse pour ramener la g̩om̩trie de la porte
                        door.transform.translate(-hingeX, 0f, 0f);
        ...
        // --- Positionnement du Joueur (H̩ros) ---
    ...
    public void render(ModelBatch modelBatch, DecalBatch decalBatch, Environment environment, PerspectiveCamera camera, float playerX, float playerZ, int playerDirection, java.util.List<fr.hibouxe.donjon_de_naheulbeuk_fan_game.model.dungeon.RoamingMonsterGroup> roamingMonsters) {
        ...
        // Vitesse d'animation ajust̩e (8) proportionnellement au mouvement
        ...
        for (fr.hibouxe.donjon_de_naheulbeuk_fan_game.model.dungeon.RoamingMonsterGroup mg : roamingMonsters) {
            ...
            mgSprite.lookAt(new com.badlogic.gdx.math.Vector3(camera.position.x, mgSprite.getY(), camera.position.z), com.badlogic.gdx.math.Vector3.Y);
    ...
    public void dispose() {
        ...
        // Les textures heroTexture, monsterTexture, chestTexture, stairsTexture sont g̩r̩es par l'AssetProvider
    }
}
