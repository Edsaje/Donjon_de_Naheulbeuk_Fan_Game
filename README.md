...
Jeu de rôle textuel et d'exploration de donjon en Java, inspiré de l'univers de la saga audio *Le Donjon de Naheulbeuk*. Ce projet met en œuvre les principes fondamentaux de la programmation orientée objet (POO), le patron d'architecture MVC et l'injection de dépendances.
...
- **Gestion de la compagnie** : Gestion des caractéristiques des aventuriers (Ranger, Nain, Élfette, Magicienne, etc.) incluant PV, attaque physique/magique, défense et ressources dédiées (Rage, Mana, Énergie).
- **Système d'inventaire et coffres** : Ramassage interactif d'objets (potions de soin), consultation du sac à dos (`I`) et consommation d'objets ciblée sur les membres de l'équipe.
- **Système de combat tour par tour** : Combat tactique alternant attaques physiques et compétences spéciales contre divers types d'ennemis (Gobelins, Orcs, Squelettes).
...
L'instance unique du composant `Menu` est créée au point d'entrée (`Main.java`) et injectée via les constructeurs de `Game`, `Battle` et des entités. Aucune classe métier n'instancie de `Scanner` ou d'élément d'I/O en interne.
