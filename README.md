...
Bienvenue sur le dépôt du **Donjon de Naheulbeuk Fan Game**, un projet monumental de RPG au tour par tour. 

Ce projet est une **vitrine technologique et architecturale** : il s'agit d'un moteur de jeu **HD-2D** (mélange de modèles 3D et de sprites 2D animés type Billboard) entièrement programmé "from scratch" en Java en utilisant le framework bas niveau **LibGDX**. 
...
- **Separation Stricte** : La logique metier (`model`) est 100% decouplee de l'interface graphique (`view`). 
...
- Les listes temporaires sont pre-instanciees et nettoyees avec `.clear()` au lieu d'utiliser `new ArrayList<>()`.
